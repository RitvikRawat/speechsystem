...
vector<int> read_obs_seq(int utterance, int digit){
	string path = "input_cropped/140101059_"+to_string(utterance)+"_"+to_string(digit)+".txt";
	vector<long double> in = readtext(path);
...
void train_HMM(vector<long double> &init_pi,vector<vector<long double> > &init_A, vector<vector<long double> > &init_B,int num_of_iter,vector<long double> &pi, vector<vector<long double> > &A, vector<vector<long double> > &B, vector<vector<long double> > alpha, vector<vector<long double> > beta, vector<vector<long double> > gamma, vector<vector<long double> > &delta, vector<vector<int> > psi, vector<vector<vector<long double> > > &zi , vector<int> in_obs ){
	// int num_of_iter = 5;
	int N = A.size()-1, T = in_obs.size()-1, M = B[0].size()-1;
	for(int i = 0 ; i <= num_of_iter ; i++){
		if(i == 0){//hardcoded values
			pi = init_pi; A = init_A; B = init_B;
		}
		else{// EM algorithm
			EM_optimiser(pi, A, B, gamma, zi, in_obs);
			modify_B(B);
		}
		alpha.clear(); 
		alpha = zero_2D( T + 1 , N + 1 );
		beta.clear(); 
		beta = zero_2D( T + 1 , N + 1 );
		gamma.clear(); 
		gamma = zero_2D( T + 1 , N + 1 );
		delta.clear(); 
		delta = zero_2D( T + 1 , N + 1 );
		psi.clear(); 
		psi = zero_2D_INT( T + 1 , N + 1 );
		zi.clear(); 
		zi = zero_3D( T + 1 , N + 1 , N + 1 );

		cout << "Iter # "<<i<<"--> Prob( O | HMM_1 ) = " << forward(pi, A, B, alpha, in_obs) << endl;
		backward(A, B, beta, in_obs);
		gamma_calculator(alpha, beta, gamma);
		vector<int> seq;
		seq = viterbi(A, B, pi, delta, psi, in_obs);
		zi_calculator(A, B,alpha, beta, gamma, zi, in_obs);
		...
		fp << "------------------------------------------------------"<<endl;
		fp << endl << "pi is : " << endl;
		log_vector(pi , "log_test.txt");
		fp << endl << "A is : " << endl;
		log_2D_vector(A , "log_test.txt");
		fp << endl << "B is : " << endl;
		log_2D_vector(B , "log_test.txt");
		fp << endl << "alpha is : " << endl;
		log_2D_vector(alpha , "log_test.txt");
		fp << endl << "beta is : " << endl;
		log_2D_vector(beta , "log_test.txt");
		fp << endl << "gamma is : " << endl;
		log_2D_vector(gamma , "log_test.txt");
		fp << endl << "delta is : " << endl;
		log_2D_vector(delta , "log_test.txt");
		fp << endl << "psi is : " << endl;
		log_2D_vector_INT(psi , "log_test.txt");
...
void get_average_model(vector<vector<long double> > &A, vector<vector<long double> > &B, vector<HMM_instance*> &models){
	long double n = models.size();
	// cout << n << endl;
	for(int x = 0 ; x < A.size() ; x++){
		for(int y = 0 ; y < A[0].size() ; y++){
			for(int i = 0 ; i < models.size() ; i++){
				A[x][y] += models[i]->A[x][y];
			}
		}
	}
	for(int x = 0 ; x < A.size() ; x++){
...
{
	vector<HMM_instance*> models;

	HMM_instance *avg_model = new HMM_instance();
	avg_model->N = NO_OF_STATES;
	avg_model->M = CODE_BOOK_SIZE;

	for(int epochs = 1 ; epochs <= 2 ; epochs++)
	{

		

		for(int utterance = 1 ; utterance <= 15 ; utterance++){
			// utt dig
			vector<int> in_obs = read_obs_seq(utterance,digit);
			in_obs.insert(in_obs.begin(), -1);

			HMM_instance *temp = new HMM_instance();
			temp->N = NO_OF_STATES;
			temp->M = CODE_BOOK_SIZE;
			temp->T = in_obs.size()-1; //we have a dummy -1 in start to begin indexing from 1
			temp->A = zero_2D( temp->N + 1 , temp->N + 1 );
			temp->B = zero_2D( temp->N + 1 , temp->M + 1 );
			temp->pi = zero_1D( temp->N + 1 );
			vector<long double> init_pi = zero_1D( temp->N + 1 ); 
			vector<vector<long double> > init_A = zero_2D( temp->N + 1 , temp->N + 1 ); 
			vector<vector<long double> > init_B = zero_2D( temp->N + 1 , temp->M + 1 );
			if(epochs == 1){
				populate_pi(init_pi);
				populate_A(init_A);
				populate_B(init_B);
				...
				init_pi = avg_model->pi;
				init_A = avg_model->A;
				init_B = avg_model->B;
			}
			train_HMM(init_pi,init_A,init_B,5,temp->pi,temp->A, temp->B,temp->alpha, temp->beta, temp->gamma,temp->delta,temp->psi,temp->zi, in_obs );

			
			models.push_back(temp);

			//print_2D_vector(temp->A);
		}
		
		//avg_model->T = in_obs.size()-1; //we have a dummy -1 in start to begin indexing from 1
		avg_model->A = zero_2D( avg_model->N + 1 , avg_model->N + 1 );
		avg_model->B = zero_2D( avg_model->N + 1 , avg_model->M + 1 );
		avg_model->pi = zero_1D( avg_model->N + 1 );

		// pi done separately
		populate_pi(avg_model->pi);
		get_average_model(avg_model->A, avg_model->B, models);
		models.clear();
		cout << "finished an epoch______________________________________________________ " << endl;

		print_2D_vector(avg_model->A);
		print_2D_vector(avg_model->B);
	}
	string fnm = "A/A_"+to_string(digit)+".txt";
	log_2D_vector(avg_model->A,fnm);
	fnm.clear();
	fnm = "B/B_"+to_string(digit)+".txt";
	log_2D_vector(avg_model->B,fnm);

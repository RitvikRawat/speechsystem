...
vector<vector<long double> > readCSV(string s){
    ...
    while (getline(infile, line)){
        ...
        string word = "";
		while (getline(strstr,word, ',')) row.push_back(stold(word));
		A.push_back(row);
		row.clear();
    }
	return A;
...
vector<vector<long double> > indexing1(vector<vector<long double> > V){
	int n = V[0].size(), m = V.size();
	vector<vector<long double> > X( m+1 , vector<long double> (n+1,0) );
	for(int i = 1 ; i <= m ; i++){
		for(int j = 1 ; j <= n ; j++){
			X[i][j] = V[i-1][j-1];
		}
...
vector<vector<long double> > read_txt_mat(string s){
	vector<vector<long double> > A;
	vector<long double> row;
	ifstream infile( s.c_str() );
    string line = "";
...
for(utterance = 1 ; utterance <= 15 ; utterance++){
	for(digit = 0 ; digit <= 9 ;digit++){
		...
		for(int d = 0 ; d <= 9 ; d++){
			HMM_instance *temp_model = new HMM_instance();
			temp_model->N = NO_OF_STATES;
			temp_model->M = CODE_BOOK_SIZE;
			temp_model->T = in_obs.size()-1; //we have a dummy -1 in start to begin indexing from 1
			vector<vector<long double> > Ax = read_txt_mat("A/A_"+ to_string( d )+".txt");
			vector<vector<long double> > Bx = read_txt_mat("B/B_"+ to_string( d )+".txt");
			temp_model->pi = zero_1D( temp_model->N + 1 );
			populate_pi(temp_model->pi);
			temp_model->A = indexing1(Ax);
			temp_model->B = indexing1(Bx);
			temp_model->alpha = zero_2D( temp_model->T + 1 , temp_model->N + 1 );;
			// print_2D_vector(temp_model->B);
			models.push_back(temp_model);
		}
	
		vector<long double> class_prob;
		for(int d = 0 ; d <= 9 ; d++){
			long double p = forward(models[d]->pi,models[d]->A,models[d]->B,models[d]->alpha, in_obs);
			class_prob.push_back(p);
		}
	
		int label = 0; long double min_p = class_prob[0];
		for(int i = 0 ; i <= 9 ; i++){
			if(min_p < class_prob[i]){
				min_p = class_prob[i]; label = i;
			}
		}
		cout <<"You spoke : "<< label << endl;
	}
}
